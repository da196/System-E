...
@Controller
public class UploadPassportSizeController {
	...
	@RequestMapping(value = "/uploadPassportSize.htm", method = RequestMethod.POST)
	public String UploadPassportSize(Model model,
			@ModelAttribute("uploadPassportSize") @Valid EmployeeEntity employeeEntity,
		...
		if (fileUpload.isEmpty()) {
			redirectAttributes.addFlashAttribute("error", "Please Upload Passport Size");			
		} else {
...
//				List<String> photoUri = new ArrayList<>();
				if (isFileUpload == true) {
					...
					employeeEntity.setPicture(AppConstants.PHOTO_URL + newFileName);
				
					
//					String photoUri = AppConstants.URL + newFileName;
//					photoUri.add(AppConstants.URL + newFileName);
					ResponseEntity<EmployeeEntity> result = employeeInfoService.uploadEmployeePassportSize(token, employeeEntity);
